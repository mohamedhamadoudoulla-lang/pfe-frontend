...
export function getImageProduit(produit, type) {
  ...
  const category = produit?.category || '';
  if (category && CATEGORY_IMAGES[category]) {
    const liste = CATEGORY_IMAGES[category];
    const id = produit?._id || produit?.id || '';
    const index = id ? id.charCodeAt(id.length - 1) % liste.length : 0;
    return liste[index] || FALLBACK;
  }

  const liste = TYPE_IMAGES[type] || TYPE_IMAGES.maison;
  const id = produit?._id || produit?.id || '';
  const index = id ? id.charCodeAt(id.length - 1) % liste.length : 0;
  return liste[index] || FALLBACK;
}
