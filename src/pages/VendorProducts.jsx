...
import { Edit3, Trash2, Plus, X, Check, Package, Hammer, Sofa, User, Mail, Phone, MapPin } from "lucide-react";
...
const CATEGORIES_AMEUBLEMENT = [
  { value: "chambre", label: "Chambre" },
  { value: "salon", label: "Salon" },
  { value: "cuisine", label: "Cuisine" },
  { value: "salle_de_bain", label: "Salle de bain" },
  { value: "salle_a_manger", label: "Salle a manger" },
  { value: "bureau", label: "Bureau" },
  { value: "exterieur", label: "Exterieur" },
  { value: "autre_ameublement", label: "Autre" },
];
...
export default function VendorProducts() {
  ...
  const categories = activeTab === "materiaux" ? CATEGORIES_CONSTRUCTION : CATEGORIES_AMEUBLEMENT;
  const filteredProducts = products
    .filter((p) => p.categorie === (activeTab === "materiaux" ? "materiaux" : "ameublement"))
    .filter((p) => !searchTerm || p.nom.toLowerCase().includes(searchTerm.toLowerCase()) || p.type.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const res = await API.post("/products", { ...form, categorie: activeTab === "materiaux" ? "materiaux" : "ameublement" });
  ...
  const totalValue = filteredProducts.reduce((s, p) => s + (p.prixUnitaire * (p.stock || 0)), 0);
  ...
  return (
    <div className="vp-page">
      ...
      <div className="vp-header">
        <h1>Mes Produits</h1>
        <p>Gerez vos produits de construction et d'ameublement</p>
      </div>
      ...
      <div className="vp-container">
        ...
        <div className="vp-tabs">
          ...
          <button className={`vp-tab ${activeTab === "ameublement" ? "active" : ""}`} onClick={() => { setActiveTab("ameublement"); setShowForm(false); setEditingId(null); }}>
            <Sofa size={16} /> Produits d'ameublement
            <span className="vp-tab-count">{products.filter((p) => p.categorie === "ameublement").length}</span>
          </button>
        </div>
        ...
        <div className="vp-stats">
          <div className="vp-stat">
            ...
            <div className="vp-stat-label">Produits</div>
          </div>
          <div className="vp-stat">
          ...
          <div className="vp-stat">
            ...
            <div className="vp-stat-label">Valeur du stock</div>
          </div>
        </div>

        <div className="vp-bar">
          <div>
            <h2>{activeTab === "materiaux" ? "Produits de construction" : "Produits d'ameublement"}</h2>
            <div className="vp-bar-meta">{filteredProducts.length} produit(s)</div>
          </div>
        ...
        {showForm && (
          <div className="vp-form-card">
            <div className="vp-form-title">
              ...
              Nouveau produit - {activeTab === "materiaux" ? "Construction" : "Ameublement"}
            </div>
                ...
                {activeTab === "ameublement" && (
                  <>
                    ...
                    <div className="vp-form-group">
                      ...
                      <input placeholder="Ex: Maison Du Batiment" value={form.shopName} onChange={(e) => setForm({ ...form, shopName: e.target.value })} required={activeTab === "ameublement"} />
                    </div>
                    <div className="vp-form-group">
                      <label>Email de la boutique</label>
                      <input type="email" placeholder="contact@boutique.tn" value={form.shopEmail} onChange={(e) => setForm({ ...form, shopEmail: e.target.value })} required={activeTab === "ameublement"} />
                    </div>
                    <div className="vp-form-group">
                      <label>Adresse de la boutique</label>
                      <input placeholder="Rue, ville, governement" value={form.shopAddress} onChange={(e) => setForm({ ...form, shopAddress: e.target.value })} required={activeTab === "ameublement"} />
                    </div>
                    <div className="vp-form-group">
                      <label>Telephone de la boutique</label>
                      <input placeholder="XX XXX XXX" value={form.shopPhone} onChange={(e) => setForm({ ...form, shopPhone: e.target.value })} required={activeTab === "ameublement"} />
                    </div>
                  </>
                )}
              </div>
        ...
        {filteredProducts.length === 0 ? (
          <div className="vp-empty">
            ...
            <p>Aucun produit {activeTab === "materiaux" ? "de construction" : "d'ameublement"}.</p>
            <p style={{ fontSize: 12, marginTop: 4 }}>Ajoutez vos premiers produits.</p>
          </div>
        ) : (
          <div className="vp-product-grid">
            {filteredProducts.map((p) => {
              if (editingId === p._id) {
                return (
                  <div key={p._id} className="vp-product-card editing">
                    <div className="vp-edit-section">
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#3b82f6", marginBottom: 8 }}>Modifier le produit</div>
                      <div className="vp-edit-row">
                        <input className="vp-edit-input" value={editForm.nom} placeholder="Nom" onChange={(e) => setEditForm({ ...editForm, nom: e.target.value })} />
                      </div>
                      <div className="vp-edit-row">
                        <select className="vp-edit-input" value={editForm.type} onChange={(e) => setEditForm({ ...editForm, type: e.target.value })}>
                          <option value="">Type</option>
                          {categories.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
                        </select>
                        <input className="vp-edit-input" type="number" min="0" step="0.01" value={editForm.prixUnitaire} placeholder="Prix" onChange={(e) => setEditForm({ ...editForm, prixUnitaire: e.target.value })} />
                      </div>
                      <div className="vp-edit-row">
                        <input className="vp-edit-input" value={editForm.unite} placeholder="Unite" onChange={(e) => setEditForm({ ...editForm, unite: e.target.value })} />
                        <input className="vp-edit-input" type="number" min="0" value={editForm.stock} placeholder="Stock" onChange={(e) => setEditForm({ ...editForm, stock: e.target.value })} />
                      </div>
                      <div className="vp-edit-row">
                        <div className="vp-scenario-btns">
                          {SCENARIOS.map((s) => (
                      ...
                      <div className="vp-edit-actions">
                        <button className="vp-btn-save" onClick={() => saveEdit(p._id)}><Check size={12} /> Enregistrer</button>
              ...
              return (
                <div key={p._id} className="vp-product-card">
                  <div className="vp-card-header">
                    <span className="vp-card-type">{categories.find((c) => c.value === p.type)?.label || p.type}</span>
                    <div className="vp-card-actions">
                      <button className="vp-btn-edit" onClick={() => startEdit(p)} title="Modifier"><Edit3 size={12} /></button>
                      <button className="vp-btn-delete" onClick={() => handleDelete(p._id)} title="Supprimer"><Trash2 size={12} /></button>
                    </div>
                  </div>
                  <div className="vp-card-body">
                    <div className="vp-card-name">{p.nom}</div>
                    <div className="vp-card-desc">{p.description}</div>
                    <div className="vp-card-meta">
                      <div className="vp-card-price">{p.prixUnitaire?.toLocaleString()} DT <span className="vp-card-unit">/{p.unite}</span></div>
                      <div className="vp-card-stock">Stock: {p.stock || 0}</div>
                    </div>
                    {p.scenario?.length > 0 && (
